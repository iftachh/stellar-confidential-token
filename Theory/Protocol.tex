...
\begin{functionality}[\fConfTran: Confidential transactions]\label{func:ConfidentialTransactions}~
	...
	\begin{description}
		...
		\begin{enumerate}
			...
			\item \Assert{$x\in (\psizeCns) \sland \tcount_d\le  \pcountCns$}.
		...
		\begin{enumerate}
			\item \Assert{$ x\in (\psizeCns) \sland \tcount_d\le  \pcountCns  \sland \balance_s  \ge x $}.
...
\item[Operation:] ~
		...
		\item Broadcast $(\init,\sset{\pk_i,\oA_i,\oP_i}_{i\in [n]})$.\footnote{Since all ciphertext are known constants, \Cc can skip actually broadcasting them.}
...
\begin{protocol}[\pConfTran.Mint]~\label{prot:ConfidentialTransactions:Mint}
	...
	\item[Operation:]  \Cc~
		\begin{enumerate}
			\item \Assert{$d\in [n] \sland \tcount_d  \le \pcountCns \sland x\in (\psizeCns)$}
...
\item[Operation:] ~
	...
	\begin{enumerate}
		...
		\item $\pi^\RpSkS \getsr \pZK{\rRpSk}((\pk_s,\oA_s- \oX_s, q/2),\sk_s)$.\footnote{Since  the protocol enforces that there  are at most  $q/\psizeCns$  increments of $\oA_s$, it holds that  $\oA_s$ is smaller than $q/2$. So the Since $\psizeCns \le q/2$ as well, this check is equivalent to ``$\oA_s- \oX_s$ encrypts a non-negative value''. \Inote{Rephrase}} 
		...
		\begin{enumerate}
			...
			\item  $\Assert{\tcount_d \le \pcountCns}$.
