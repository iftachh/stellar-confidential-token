...
\newcommand{\tcount}{\mathsf{pndCount}}
\newcommand{\gcount}{\mathsf{gCount}}
...
\newcommand{\Auditor}{\Ac}
\newcommand{\Chain}{{\MathAlgX{C}}}

\newcommand{\pcountCns}{p_{\mathsf{count}} }
\newcommand{\psizeCns}{p_{\mathsf{size}} }
\newcommand{\gcountCns}{g_{\mathsf{count}} }
...
\newcommand{\rRpDef}
{
	\sset{((\pk,\oA,\cS),(a,r))\colon \Enc_\pk(a;r)= \oA \sland a\in \cS}
}
...
\newcommand{\rRpSkDef}
{
	\sset{((\pk,\oA,\cS),w)\colon \KeyGen(w) = (\sk,\pk) \sland  \Dec_\sk(\oA) \in \cS}
}
...
\begin{functionality}[\fConfTran: Confidential transactions]\label{func:ConfidentialTransactions}~
	...
	\item[Parties:]	Mint \Mc, chain holder  \Cc, auditor \Auditor,  and users  $\Uc_1, 
	\ldots,\Uc_n$.
	
	
	\item[Parameters:] $\pcountCns,\psizeCns,\gcountCns, q \in \N$.
	...
	\begin{description}
		...
		\begin{enumerate}
			...
			\item \Assert{$x\in (\psizeCns) \sland \tcount_d\le  \pcountCns \sland \gcount_d\le  \gcountCns$}.
			
			\item $\tcount_d^{++},\gcount_d^{++}$.
		...
		\begin{enumerate}
			\item \Assert{$ x\in (\psizeCns) \sland \tcount_d\le  \pcountCns  \sland \gcount_d\le  \gcountCns  \sland \balance_s  \ge x $}.
		...
		%	\item $\Assert{\balance_i \le q/4}$.\footnote{This check is to prevent overflow. In reality, it will never fire.}
	...
	%	\item[History.]     Upon receiving $\history$ from  party $\Uc_i$ and \Cc:   Send $(\historylog,\historylog_i)$ to  $\Uc_i$.	 
		...
		\item[Audit.]     Upon receiving $\audit$ from  \Auditor, send $(\transfer,s,d,x)$ to \Auditor, for each transfer command that happened since  the init call.
...
\item[Parameters:] $\pcountCns,\psizeCns,q \in \N$.
...
This sub-protocol is were the encryption key are sampled and shared, and the chain manager \Cc set the initial balances  of all users to zero. The protocol  uses ZKPOK proof for the relation:
\begin{description}
...
\item[Operation:] ~
			...
			\begin{enumerate}
				...
				\end{enumerate}
					...
					\begin{enumerate}
					\item 		Generate $(\pk_A,\sk_A)$ as above.
					
					\item Send $\pk_A$ to \Cc.
				...
				\begin{enumerate}
					...
					\item $\oP_i\getsr \Enc_{\pk_i}(0;0),\oA_i\getsr \Enc_{\pk_i}(0;0),\tcount_i,\gcount_i 
					\asn 0$. 
					
			
				\end{enumerate}
			
		\item Broadcast $(\init,\sset{\pk_i,\oA_i,\oP_i}_{i\in [n]},\pk_A)$.\footnote{Since all ciphertexts are known constants, \Cc can skip actually broadcasting them.}
...
\begin{protocol}[\pConfTran.Mint]~\label{prot:ConfidentialTransactions:Mint}
	...
	\item[Common input:]  $d\in [n]$ and $x\in (\psizeCns)$.
	
	\item[Operation:]  \Cc~
		\begin{enumerate}
			\item \Assert{$d\in [n] \sland \tcount_d  \le \pcountCns \sland \gcount_d  \le \gcountCns \sland x\in (\psizeCns)$}
...
\begin{description}
	...
	Namely, encryption of values in $[\psizeCns]$, witness is random coins.
	
		\item[In range using secret key.] $\rRpSk =\rRpSkDef$.\label{rel:RpSk}
		
		
		Namely, encryption of values in $[\psizeCns]$, witness is secret key. 
		\begin{remark}
...
%\item[Algorithms:]    \Dec.
	...
	\item[Common input:] $d\in [n]$. \comment{Payee (destination) name.}
	
	\item[$\Uc_s$'s private  input:] $x \in (\psizeCns)$.
...
\item[Operation:] ~
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item $\pi^\RPs \getsr \pZKPOK{\rRp}((\pk_s,\oX_s,(\psizeCns)),(x,r_s))$.
		...
		\item $\pi^\Eqs_d \getsr \pZK{\rEq}((\pk_s,\pk_d,\oX_s, \oX_d),(x,r_s,r_d))$.
			
			
			\item $\oX_A \getsr \Enc_{\pk_A}(x;r_A)$ for $r_A \getsr \cD$.
		
			\item $\pi^\Eqs_A \getsr \pZK{\rEq}((\pk_s,\pk_A,\oX_s, \oX_A),(x,r_s,r_A))$.
		
		
			
		
			
		\item $\pi^\RpSkS \getsr \pZK{\rRpSk}((\pk_s,\oA_s- \oX_s, (q/2)),\sk_s)$.\footnote{Since  the protocol enforces that there  are at most  $\gcountCns$  increments of $\oA_s$, it holds that  $\oA_s$ is smaller than $q/2$. Hence, this check is equivalent to ``$\oA_s- \oX_s$ encrypts a non-negative value''.} 
			
			
			\item Send $(\oX_s,\oX_d,\oX_A,\pi^\RPs,\pi^\Eqs_d,\pi^\Eqs_A,\pi^\RpSkS)$ to \Cc.   
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\item  $\Assert{\tcount_d \le \pcountCns \sland \gcount_d  \le \gcountCns}$.
				
			\item   $\vZKPOK{\rRp}((\pk_d,\oX_d,(\psizeCns)),\pi^\RPs)$,
			
			 $\vZK{\rEq}((\pk_s,\pk_d,\oX_s,\oX_d),\pi^\Eqs_d)$,
			 
			  $\vZK{\rEq}((\pk_s,\pk_A,\oX_s,\oX_A),\pi^\Eqs_A)$
		    ...
		    % \item  $\historylog \unioneq (\transfer,s,d,\oA_s,\oP_d)$.
		     ...
		     \item Broadcast $(\transfer,s,d,\oP_d,\oX_A)$.
		\end{enumerate}
...
\begin{protocol}[\pConfTran.Withdraw]~\label{prot:ConfidentialTransactions:Withdraw}
	\item[Parties:] $\Uc_i$ and \Cc.
	
	
	
	\item[Common input:]  $x\in[q/4]$.
	
	\item[Operation:]  ~
	
	\begin{enumerate}
		
		\item  $\Uc_i$:  Send $\pi^\RpSkS \getsr \pZKPOK{\rRpSk}((\pk_i,\oA_i,[x,q/2]),\sk_i)$ to \Cc.
	
	
	\item  \Cc: 
	\begin{enumerate}
		\item Call $\vZKPOK{\rRpSk}((\pk_i,\oA_i,[x,q/2]),pi^\RpSkS)$.
		
		\item Let $\oA_i \minuseq \Enc_{\pk_i}(x;0)$.
		
		\item Broadcast $(\withraw,i,x)$
	\end{enumerate}
\end{enumerate}
\end{protocol}
...
\begin{protocol}[\pConfTran.Audit]~\label{prot:ConfidentialTransactions:Audit}
	\item[Parties:] \Auditor.
	
	
	
	
	\item[Operation:]   For each  published transfer operation   $(\transfer,s,d,\oP_d,\oX_A)$,  \Auditor
	
	\begin{enumerate}
		\item Let $x \asn \Dec_{\sk_A} (\oX_A)$.
		
		\item Output $((\transfer,s,d,x))$.
	\end{enumerate}
...
Assuming  $(\KeyGen,\Enc,\Dec)$ is CPA secure, then \cref{prot:ConfidentialTransactions} UC-realizes \cref{func:ConfidentialTransactions} against semi-honest chain holder and mint.
\end{theorem}
