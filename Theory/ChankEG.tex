...
\begin{theorem}[Security  of twisted-ElGamal in-the-exponent]\label{thm:tEGinExp}
	Assuming DDH is hard over $\cG$, then \cref{alg:tEGinExp} is  a perfectly binding,  semantically secure additively homomorphic scheme over $\Z_q$, with the following caveat:  the decryption  only guaranteed to work on encryptions of plaintexts in $(b)$ ($1^b$ is the parameter of the key-generation algorithm)..
\end{theorem}
...
The chunk ElGamal encryption scheme  is defined as follows:


\newcommand{\cs}{c}
\begin{notation}
Fix $t,\cs\in \N$ with $t\le q$ and $\ell \asn t/c \in \N$. 	
\end{notation}

\begin{definition}[Base factorization \basefactor]\label{def:basefactor}
For $a\in (t)$ let $\basefactor(a) \eqdef (a_0,\ldots,a_{\ell-1})$ so that $a =  \sum_{i \in (\ell)}    2^{i \cs} \cdot a_i$.
\end{definition}
...
\begin{algorithm}[$(\KeyGen,\Enc,\Dec)$: Chunk ElGamal adaptively homomorphic encryption]\label{alg:ChunkEGa}~
	...
	\item[Decription:] $\Dec_\sk(\oM)$   
	\begin{enumerate}
		\item 	 For each $i\in (\ell)$: let $m_i \getsr \EgDec_\sk(\oM_i)$.
		
		\item Let  $m \asn \sum_{i \in (\ell)}   2^{i \cs} \cdot  m_i $.
...
\begin{theorem}[Security  of Chunk ElGamal]\label{thm:ChunkEG}
	Assuming \DDH is hard over $\cG$, then \cref{alg:ChunkEGa} is  a perfectly binding,  semantically secure additively homophobic scheme over $\Z_q$, with the following caveats
	\begin{enumerate}
		
		\item 	Encryption  algorithm only encrypts values in $(t)$.
		
		 
		\item Decryption algorithm only decrypts encryptions of a plaintext  $a$ with $\basefactor(a)\in (b)^\ell$ ($1^b$ is the parameter of the key-generation algorithm).
	\end{enumerate}
...
\begin{description}
	...
	\begin{enumerate}
		...
		\item Let  $a \asn  \sum_{i\in (\ell)}    2^\cs \cdot \oa_i$. 
		
		\item For both $j\in \zo$:  $\tA_j \asn  \sum_i  2^\cs \cdot (\oA_j)_i$. 
		
		  
		\item    $r_1 \asn  \sum_{i\in (\ell)}  2^\cs \cdot (\orr_1)_i$. 
...
\begin{description}
	...
	\begin{description}
		...
		\begin{enumerate}
			...
			\item    For both $j\in \zo$:   $\tA_j \asn  \sum_{i\in (\ell)}    2^\cs \cdot (\oA_j)_i$. 
			
			\item  Let  $\pi^\EgEqSkS \getsr \pZK{\rEgEqSk}((E,\tA_0,\tA_1), e)$.
...
\begin{description}
	...
	\begin{enumerate}
			\item   $a \asn  \sum_{i\in (\ell)}   2^\cs \cdot \oa_i$. 
		\item    $\tA \asn  \sum_{i\in (\ell)}    2^\cs \cdot \oA_i$. 
		\item    $r \asn  \sum_{i\in (\ell)}  2^\cs \cdot \orr_i$. 
...
\begin{description}
	...
	\begin{description}
		...
		\begin{enumerate}
			...
			\item   For each $i\in  (\ell)$, let $a_i \asn  \Dec_e(\tA_i)$.
			
			
			\item  Let  $a\asn  \sum_{i\in (\ell)}    2^\cs \cdot a_i$. 
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item 	Each $\Uc_i$ sets the parameters of the encryption key generation algorithm to $(1^b,H)$, for  $b \asn 2^{c}\cdot \pcountCns$.

	\end{enumerate}
 
	\item[Transfer.]   In addition to the original protocol, $\Uc_s$
	\begin{enumerate}
		\item Provides a proof that $X_d$ is fresh, \ie using \pZK{\rtFsh} with parameter $b\asn 2^\cs$.
...
In this section, we present a distributed key-generation  protocol for the key generation algorithm \tdKg described above.  We make use of the following two-party multiplication   functionality.
